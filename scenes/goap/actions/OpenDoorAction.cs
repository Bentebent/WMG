...
namespace wmg
{
    public class OpenDoorAction<T> : BaseMoveAction<T>
        where T : Node3D, INavigatable3D
    {
        private Door _targetDoor = null;

        public OpenDoorAction(string name, GameManager gameManager, Door targetDoor)
        ...
        {
            return new OpenDoorAction<T>(Name, _gameManager, _targetDoor);
        }
    }
}
